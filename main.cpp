...
void display(char text[],int x,int y){
    int i;
    glColor3i(0,0,0);
    glRasterPos2i(x,y);
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    {
        glClearColor(0.078, 0.094, 0.322,1.0f);
...
void check(){
    switch(count_chap){
    case 1:
            PlaySound("output1.wav", NULL, SND_ASYNC|SND_FILENAME|SND_LOOP);
            text1_visibility=false;
            break;
    case 2:PlaySound("output2.wav", NULL, SND_ASYNC|SND_FILENAME|SND_LOOP);
            break;
    case 3:PlaySound("output3.wav", NULL, SND_ASYNC|SND_FILENAME|SND_LOOP);
            break;
    case 4:PlaySound("output4.wav", NULL, SND_ASYNC|SND_FILENAME|SND_LOOP);
            break;
    case 5:PlaySound("output5.wav", NULL, SND_ASYNC|SND_FILENAME|SND_LOOP);
            break;
    }
}
...
void submenu(int op)
{
    ...
    {
        PlaySound("Casper The Friendly Ghost - Little Richard.wav", NULL, SND_ASYNC|SND_FILENAME|SND_LOOP);
        count_chap=0;
    }
     else if(op == 3)
    {
        PlaySound("Deep Boo Sea.wav", NULL, SND_ASYNC|SND_FILENAME|SND_LOOP);
        count_chap=0;
    }
    else if(op == 3)
        exit(0);
...
void magical_effect(){
    ...
    for(int i=0;i<1000;++i)
        glVertex2f(cos(2*3.14159*i/1000.0)*r1+50,sin(2*3.14159*i/1000.0)*r1+71);
    glEnd();

    glBegin(GL_POLYGON);
    for(int i=0;i<1000;++i)
    glVertex2f(cos(2*3.14159*i/1000.0)*r2+30,sin(2*3.14159*i/1000.0)*r2+38);
    glEnd();
    glBegin(GL_POLYGON);
    for(int i=0;i<1000;++i)
    glVertex2f(cos(2*3.14159*i/1000.0)*r3+0,sin(2*3.14159*i/1000.0)*r3+15);
    glEnd();
    glBegin(GL_POLYGON);
    for(int i=0;i<1000;++i)
    glVertex2f(cos(2*3.14159*i/1000.0)*r4+75,sin(2*3.14159*i/1000.0)*r4+12);
    glEnd();
    glBegin(GL_POLYGON);
    for(int i=0;i<1000;++i)
    glVertex2f(cos(2*3.14159*i/1000.0)*r5+90,sin(2*3.14159*i/1000.0)*r5+40);
    glEnd();
    glBegin(GL_POLYGON);
    for(int i=0;i<1000;++i)
    glVertex2f(cos(2*3.14159*i/1000.0)*r6+90,sin(2*3.14159*i/1000.0)*r6+75);
    glEnd();

}
void chandamama(){
    if(flag==0)
        glColor3f(1.0f,1.0f,0.0f);//sunny day
    else if(flag==1)
        glColor3f(0.5f,0.5f,0.5f);//night time
    glBegin(GL_POLYGON);
    for(int i=0;i<1000;++i)
    glVertex2f(cos(2*3.14159*i/1000.0)*5+90,sin(2*3.14159*i/1000.0)*5+85);
    glEnd();
}
void myDisplay()
{
    ...
    ///cloud end
    if(text1_visibility){
        display("CASPER GHOST",-7,60);
        display("ABHILASH KUMAR SINGH",-10,55);
        display("1BI17CS002-  A1",-10,50);
        display("ANCHAL M",-10,45);
        display("1BI16CS221-  A4",-10,40);
    }
    if(disp_time>210000)
        text1_visibility=false;
    ...
    switch(count_chap){
        case 1:display("SCENE 1",0,80);
            display("AUDIOBOOK",45,98);
            display("  FOR PREVIOUS<-  ->FOR NEXT",0,75);
            display("CASPER IS BORED!!!",0,70);
            break;
        case 2:display("SCENE 2",0,80);
            display("AUDIOBOOK",45,98);
            display("  FOR PREVIOUS<-  ->FOR NEXT",0,75);
            display("WAY TO EARTH",0,70);
            break;
        case 3:display("SCENE 3",0,80);
            display("AUDIOBOOK",45,98);
            display("  FOR PREVIOUS<-  ->FOR NEXT",0,75);
            display("SURPRISE",0,70);
            break;
        case 4:display("SCENE 4",0,80);
            display("AUDIOBOOK",45,98);
            display("  FOR PREVIOUS<-  ->FOR NEXT",0,75);
            display("IT SHOULDN'T HAPPEN",0,70);
            break;
        case 5:display("SCENE 5",0,80);
            display("AUDIOBOOK",45,98);
            display("  FOR PREVIOUS<-  LAST SCENE,\nQUIT OR SEE PREVIOUS SCENE",0,75);
            display("MORAL",0,70);
            break;
    }
    ///glVertex2f(,);
...
int main()
{
   ...
   glutInitWindowPosition(260,0);
    ...
    glutIdleFunc(idle);
   ///Make sub-menu
    int iSubMenu = glutCreateMenu(submenu);
	glutAddMenuEntry("Classic", 1);
	glutAddMenuEntry("Little Richard", 2);
	glutAddMenuEntry("Deep Boo Sea", 3);

    ///Make main-menu
    int iMainMenu = glutCreateMenu(menu);
	glutAddSubMenu("Choose Music", iSubMenu);
